...
(function (root) {
  ...
  function BoyMemory(storageKey) {
    this.storageKey = storageKey;
    this.data = defaultMemory();
    this._sessionStart = Date.now();
  }

  BoyMemory.prototype.load = function () {
    var stored = BoyCompanion.Storage.get(this.storageKey, null);
  ...
  BoyMemory.prototype.save = function () {
    this.data.totalTimeMs += (Date.now() - this._sessionStart);
    this._sessionStart = Date.now();
    return BoyCompanion.Storage.set(this.storageKey, this.data);
  };

  BoyMemory.prototype.recordSectionVisit = function (sectionId) {
    if (!sectionId) return;
    this.data.sectionVisits[sectionId] = (this.data.sectionVisits[sectionId] || 0) + 1;
    this.data.lastSection = sectionId;
  };

  BoyMemory.prototype.recordSkyVisit = function (skyName) {
    if (!skyName) return;
    this.data.skyVisits[skyName] = (this.data.skyVisits[skyName] || 0) + 1;
  };

  BoyMemory.prototype.collectFlower = function () { this.data.flowersCollected++; };
  BoyMemory.prototype.collectStar = function () { this.data.starsCollected++; };
  BoyMemory.prototype.placeHeart = function () { this.data.heartsPlaced++; };

  BoyMemory.prototype.findSecret = function (id) {
    var isNew = !this.data.secretsFound[id];
    this.data.secretsFound[id] = true;
    return isNew;
  };

  BoyMemory.prototype.setMood = function (mood) { this.data.lastMood = mood; };

  BoyMemory.prototype.favoriteSection = function () {
    return topKey(this.data.sectionVisits);
  };

  BoyMemory.prototype.favoriteSky = function () {
    return topKey(this.data.skyVisits);
  };

  BoyMemory.prototype.isReturningVisitor = function () {
