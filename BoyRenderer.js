...
(function (root) {
  ...
  function BoyRenderer(atlasImage, container) {
  ...
  BoyRenderer.prototype._resize = function () {
    this.dpr = window.devicePixelRatio || 1;
  };
   ...
   */
  BoyRenderer.prototype.draw = function (frameRect, pos, flipped) {
    if (!frameRect) return;
  ...
  BoyRenderer.prototype.lastSize = function () {
    return { w: this._lastDestW || Config.RENDER_HEIGHT_PX, h: this._lastDestH || Config.RENDER_HEIGHT_PX };
  };

  BoyRenderer.prototype.setVisible = function (visible) {
    this.canvas.style.display = visible ? '' : 'none';
  };

  BoyRenderer.prototype.destroy = function () {
    window.removeEventListener('resize', this._boundResize);
    if (this.canvas.parentNode) this.canvas.parentNode.removeChild(this.canvas);
  };

  BoyCompanion.Renderer = BoyRenderer;

  /** Loads the atlas image + manifest JSON. Returns a Promise. */
  BoyCompanion.loadAssets = function () {
