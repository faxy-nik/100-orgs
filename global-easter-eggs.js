...
(function () {
  ...
  function openSecretDoor() {
    ...
    if (!reduce) {
      for (var i = 0; i < 36; i++) {
        stars += '<i style="position:absolute;left:' + (Math.random() * 100).toFixed(1) + '%;top:' + (Math.random() * 100).toFixed(1) + '%;width:2px;height:2px;border-radius:50%;background:#ffe680;opacity:' + (0.4 + Math.random() * 0.6).toFixed(2) + ';animation:ashTwinkle ' + (1.5 + Math.random() * 2).toFixed(2) + 's ease-in-out ' + (Math.random() * 1.5).toFixed(2) + 's infinite;"></i>';
      }
    }
    overlay.innerHTML =
      '<style>@keyframes ashTwinkle{0%,100%{opacity:.1}50%{opacity:1}}</style>' + stars +
